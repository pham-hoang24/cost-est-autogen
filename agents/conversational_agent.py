...
def build_conversational_agent(llm_config, session_id: str = None) -> ConversableAgent:
    ...
    system_message = (
        ...
        "      - If the user just approved (e.g. said 'correct', 'yes', 'looks good'), call `confirm_expansion_tool(project_id, approval_text=\"approve\")`\n"
        "      - If the user requested changes, call `submit_user_description_tool` with the new details, then re-draft.\n"
        "      - If you haven't shown the draft yet, show it and ask for confirmation.\n"
