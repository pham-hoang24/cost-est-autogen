...
def build_conversational_agent(llm_config, session_id: str = None) -> ConversableAgent:
    ...
    system_message = (
        ...
        "   b) If status is 'BASELINE_COLLECTED':\n"
        ...
        "      - CRITICAL: Analyze the user's latest message.\n"
        "      - If the user indicates approval (e.g., \"correct\", \"yes\", \"looks good\", \"proceed\", \"approve\"), YOU MUST call `confirm_expansion_tool(project_id, approval_text=\"approve\")` IMMEDIATELY. Do not re-summarize.\n"
        "      - If the user requested changes, call `submit_user_description_tool` with the new details, then re-draft.\n"
        "      - If the user's message is unclear or you haven't shown the draft yet, show the expansion summary and ask: \"Does this look correct?\"\n"
