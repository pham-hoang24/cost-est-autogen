...
def build_conversational_agent(llm_config, session_id: str = None) -> ConversableAgent:
    ...
    system_message = (
        ...
        "      - If baseline is complete → Skip to step 7 to handle description, expansion, and then method evaluation.\n"
        ...
        "   NOTE: When you 'request InterpreterAgent to parse and store', you actually do that by calling `intake_step(session_id=project_id, user_text=<user message>)`.\n\n"
        ...
        "7) Once all 5 baseline fields are collected (missing_baseline is empty), coordinate the workflow. For ANY new non-greeting user message at this point, treat it as the authoritative project description for expansion, even if user_description already has a placeholder.\n"
        "   a) Check the project status in the context.\n"
        "   b) If status is 'BASELINE_COLLECTED' and `expansion_draft` is missing (None):\n"
        "      - The user's CURRENT message is their project description.\n"
        "      - YOU MUST FIRST call `submit_user_description_tool(project_id, description=<user_message>)` to overwrite any placeholder description.\n"
        "      - THEN call `draft_expansion_tool(project_id)` to generate the expansion.\n"
        "      - Present the expansion draft to the user and ask for confirmation: \"I've drafted an expanded project context based on your inputs. [Show summary]. Does this look correct?\"\n"
        "      - End with \"[WAITING FOR USER INPUT]\" and wait for user response\n\n"
        "   c) If status is 'AWAITING_EXPANSION':\n"
        "      - Treat any message that is primarily confirmation as a SIGNAL TO PROCEED.\n"
        "      - Examples: \"Yes\", \"Yes, looks good\", \"Yes, proceed\", \"Yes, proceed with estimation\", \"Correct\", \"Looks good\".\n"
        "      - If the message contains \"yes\", \"correct\", or \"looks good\" and does not include words like \"change\", \"update\", or \"but\", you MUST interpret it as confirmation even if it also mentions \"proceed\" or \"estimation\".\n"
        "      - For such messages you MUST NOT re-draft or re-display the summary.\n"
        "      - Instead you MUST:\n"
        "        1) Call `confirm_expansion_tool(project_id, approval_text=\"approve\")`.\n"
        "        2) WAIT for the tool result.\n"
        "        3) CRITICAL ERROR HANDLING: If the tool returns an error, IMMEDIATELY call `get_project_context_tool(project_id)`.\n"
        "           - If the status is 'EXPANSION_CONFIRMED', ignore the error and proceed as if it succeeded.\n"
        "           - If the status is NOT 'EXPANSION_CONFIRMED', report the error.\n"
        "        4) THEN follow the instructions for status \"EXPANSION_CONFIRMED\" (show inferred metrics, request method selection, etc.).\n"
        "      - IF user requests changes: Call `submit_user_description_tool` with new details.\n"
        "      - IF unclear/no keywords: Show summary again and ask \"Does this look correct?\".\n"
        "   d) After confirmation (status becomes EXPANSION_CONFIRMED):\n"
        ...
        "   - When you receive the recommendations (or if you see `selection` in the context), you MUST output a message to the user confirming the selection.\n"
        "   - CRITICAL: Check if `selection` exists in the context (it has `primary` and `backups`).\n"
        "   - IF selection exists (regardless of whether status is METHOD_SELECTED or INPUTS_REQUESTED), you MUST append a hidden signal at the end of your message so the UI can show method cards.\n"
