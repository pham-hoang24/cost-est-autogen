...
function InvitationsPage() {
  ...
  const fetchInvitations = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:8080/api/collaboration/invitations', {
        headers: {
  ...
  const handleAcceptInvitation = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/collaboration/invitations/accept', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
