...
export default function ProfessionalCostEstimationService({
  ...
  const generateResults = async () => {
    try {
      ...
      // =========================================================================
      // Step 1: Run Estimation via /run-estimation endpoint
      // =========================================================================
      // This ensures the estimation is actually computed before generating report

      const selectedMethod = selectedMethodology[0] || "cocomo";
      console.log(
        `[generateResults] Running estimation with method: ${selectedMethod}`
      );

      const estimationResponse = await fetch(
        "http://localhost:8000/run-estimation",
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            inputs: null, // No user overrides initially
          }),
        }
      );

      if (!estimationResponse.ok) {
        const errorText = await estimationResponse.text();
        console.error("[generateResults] Estimation failed:", errorText);
        throw new Error(`Estimation failed: ${estimationResponse.status}`);
      }

      const estimationResult = await estimationResponse.json();
      console.log("[generateResults] Estimation result:", estimationResult);

      // Handle INPUTS_REQUIRED response
      if (estimationResult.status === "INPUTS_REQUIRED") {
        console.log(
          "[generateResults] Missing inputs:",
          estimationResult.missing_inputs
        );
        // For now, show an alert with missing inputs
        // TODO: Show a modal to collect missing inputs
        const missingFields = estimationResult.missing_inputs
          .map(
          ...
          .join("\n");
        alert(
          `Some inputs are required for ${selectedMethod} estimation:\n\n${missingFields}\n\nUsing default values for now.`
        );

        // Retry with default inputs marker (backend will use defaults)
        const retryResponse = await fetch(
          "http://localhost:8000/run-estimation",
          {
            ...
            body: JSON.stringify({
              session_id: sessionId,
              method_id: selectedMethod,
              inputs: { use_defaults: true }, // Signal to use defaults
            }),
          }
        );

        if (!retryResponse.ok) {
          throw new Error("Estimation with defaults also failed");
        }
      }

      // =========================================================================
      // Step 2: Generate Full Report via /generate-report endpoint
      // =========================================================================
      // Now that estimation is complete, generate the full report

      console.log("[generateResults] Generating full report...");
      ...
      console.log("[generateResults] Report generated:", data);
