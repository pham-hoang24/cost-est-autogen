...
export default function AdminConsolePage() {
  ...
  const downloadReport = () => {
    ...
    const reportContent = `
SW4E SANDBOX - PROJECT MANAGEMENT REPORT
==========================================

Generated on: ${new Date().toLocaleString()}
Report Type: Comprehensive Project Analysis
System: SW4E Research Collaboration Platform
...
DEMO VERSION - This is a demonstration report for testing purposes.
    ...
    link.href = url;
    link.download = `SW4E-Project-Report-${new Date().toISOString().split('T')[0]}.txt`;
    link.click();
...
Digital Twin for Smart Buildings,On Hold,IoT,€170K,6,14 months,0,0,2023-04-01,2024-06-01,Prof. Sanna Rantala,Tampere University,EU AI Act Compliant`;
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `SW4E-Project-Analytics-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
    window.URL.revokeObjectURL(url);
  ...
  const fetchAdminData = async () => {
    ...
    try {
      // Fetch users data
      const usersRes = await fetch('/api/simple-governance/users', {
      ...
      // Fetch projects data
      const projectsRes = await fetch('/api/collaboration/projects', {
        headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' }
      });
      ...
      const aiServicesRes = await fetch('/api/ai-services/catalog', {
        headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' }
      });
      ...
      const hardwareRes = await fetch('/api/hardware/requests', {
        headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' }
      });
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {[
                        {
                          id: 1,
                          name: 'SW4E Administrator',
                          email: 'admin@sw4e.org',
                          role: 'super_admin',
                          status: 'active',
                          organization: 'SW4E Platform',
